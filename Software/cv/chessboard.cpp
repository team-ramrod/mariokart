...
void registerChessboard(vector<CvPoint2D32f> corners, CvMat *capParams, CvMat *capDistortion);
...
void chessboard_find(CvCapture *capture){
		...
		if (chessboardPoints.size() == CHESSBOARD_WIDTH*CHESSBOARD_HEIGHT) {
			registerChessboard(chessboardPoints, capParams, 0);
		}
...
vector<CvPoint2D32f> findChessboard(IplImage *frame) {
	...
	vector<CvPoint2D32f> chessboardCorners; chessboardCorners.resize(CHESSBOARD_WIDTH*CHESSBOARD_HEIGHT);
...
void registerChessboard(vector<CvPoint2D32f> corners, CvMat *capParams, CvMat *capDistortion) {
	...
	vector<CvPoint2D32f> chessboardPoints; chessboardPoints.resize(CHESSBOARD_WIDTH * CHESSBOARD_HEIGHT);
	//Loop through each corner and add it to the vecot of points
	for (int i=0; i<CHESSBOARD_HEIGHT; i++)
        {
		for (int j=0; j<CHESSBOARD_WIDTH; j++)
                {
                    chessboardPoints[1] = cvPoint2D32f(j*SQUARE_SIZE, i*SQUARE_SIZE);
	...
	float rotVect[3]; CvMat rotationVector = cvMat(1,3, CV_32FC1, rotVect);
	float transVect[3]; CvMat translationVector = cvMat(1,3, CV_32FC1, transVect);
	...
	float rotMatrix[9]; CvMat rotationMatrix = cvMat(3,3, CV_32FC1, rotMatrix);
